...
if [ $TRANSPARENT_PROXY = 'true' ]; then
    ...
    #开启转发
    touch /etc/sysctl.d/proxy.conf
    echo "net.ipv4.ip_forward = 1" >> /etc/sysctl.d/proxy.conf && sysctl -p
    echo "net.ipv6.conf.all.forwarding = 1" >> /etc/sysctl.d/proxy.conf && sysctl -p
